...
import { SliderRotateCaptcha, SliderRotateCaptchaData } from '@web-rc/biz-components';
...
export default () => {
  ...
  async function handleLoad() {
    const maxDegree = 300, minDegree = 120;
    const ranRotate = Math.floor(minDegree + Math.random() * (maxDegree - minDegree)); // 生成随机角度
    const img = await getRotatedImage('https://unpkg.com/@vbenjs/static-source@0.1.7/source/avatar-v1.webp', ranRotate,);
    setRandomRotate(ranRotate);
    const data: SliderRotateCaptchaData = { img, degree: { min: minDegree, max: maxDegree } };
    return data;
  }

  async function handleVerify(currentRotate: number) {
