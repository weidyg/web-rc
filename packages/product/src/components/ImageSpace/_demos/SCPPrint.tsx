...
function caniaoPrint(printer: string, documents: PrintDocument[]) {
...
function getSfPrintData_CAINIAO(data: SfPrintDataRequest_CAINIAO, sbox?: boolean): Promise<SfPrintDataResonse_CAINIAO> {
  const url = sbox ? 'https://sfapi-sbox.sf-express.com/std/service' : 'https://bspgw.sf-express.com/std/service';
...
function getToken(data: SfTokenRequestData, sbox?: boolean): Promise<SfTokenResonse> {
  const url = sbox
...
function fetch_SF<SfTResonse extends SfApiBaseResonse>(url: string, options: RequestInit): Promise<SfTResonse> {
