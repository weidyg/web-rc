...
function getError<T extends UploadResponse>(
...
export default function uploadRequest<T extends UploadResponse = UploadResponse>(option: UploadRequestOption<T>) {
  const xhr = new XMLHttpRequest();
  if (option.onProgress && xhr.upload) {
    xhr.upload.onprogress = function progress(e: UploadProgressEvent) {
      const { total = 0, loaded = 0 } = e;
      if (total > 0) {
        e.percent = (loaded / total) * 100;
      }
      option.onProgress?.(e);
    };
  }
