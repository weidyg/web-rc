...
>(props: UploadDraggerProps<T>) => {
    const { buttonProps, style, normalize, configRender,
        ...
        fileList, onChange, ...restUploadProps
    } = props;
    ...
    return wrapSSR(
        <div style={{ ...style, }} className={classNames(`${prefixCls}`, hashId)}>
            <div className={classNames(`${prefixCls}-config`, hashId)}>
                {configRender?.()}
            </div>
            <Upload.Dragger
                {...uploadProps}
            ...
            >
                ...
                <p className={classNames(`${prefixCls}-board-tips`, hashId)}>
                    点击按钮或将图片拖拽至此处上传
                </p>
                <p className={classNames(`${prefixCls}-board-format`, hashId)}>
