...
const ImageDesc = (props: ImageDescProps, ref: Ref<ImageDescRef>) => {
  const { transform, actionsRender } = props;
      ...
      <div className={classNames(`${prefixCls}-preview`, hashId)}>
        ...
        <div className={classNames(`${prefixCls}-content`, hashId)}>
          ...
          <div style={{ height: '612px', overflow: 'auto', border: 'none' }}>
            {value?.map((m, i) => {
              return (
                <img key={i} style={{ width: '-webkit-fill-available', maxWidth: '360px' }} src={transform?.(m, 360) ?? m} />
              );
            })}
          </div>
        </div>
        ...
        <div className={classNames(`${prefixCls}-operate-imgs`, hashId)}>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
            {value?.map((imgurl, index) => (
              <ImageItem.Drag key={index} index={index} onDrag={dropImg}>
                <ImageItem
                  showNo={{ index }}
                  imgUrl={transform?.(imgurl, 88, 88) ?? imgurl}
                  renderActions={
                    (edit || remove) && {
                      edit: edit?.(index),
                      remove: remove?.(index),
